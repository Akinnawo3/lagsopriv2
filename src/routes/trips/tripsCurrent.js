...
const  TripsCurrent = ({match, trips, getTrips, isLoading, getTripCount, tripCount}) => {
	const [currentPage, setCurrentPage] = useState(1);
	const [postsPerPage] = useState(10);

useEffect(()=> {
	getTrips(1, 'on_trip');
	getTripCount('on_trip')
	},[])
	...
	return (
			<div className="table-wrapper">
				...
				{isLoading && <Spinner />}
				{!isLoading && <RctCollapsibleCard heading="Current Trips" fullBlock>
								...
								<Fragment>
									{trips.length > 0 && trips.map((trip) => (
										<TableRow hover key={trip._id}>
											...
											<TableCell><Badge color="secondary">current</Badge></TableCell>
											<TableCell>
