...
const  TripsCancelled = ({match, trips, getTrips, isLoading, getTripCount, tripCount}) => {
...
useEffect(()=> {
	...
	return (
			<div className="table-wrapper">
				...
				{isLoading && <Spinner />}
				{!isLoading && <RctCollapsibleCard heading="Cancelled Trips" fullBlock>
								...
								<Fragment>
									{trips.length > 0 && trips.map((trip) => (
										<TableRow hover key={trip._id}>
											...
											<TableCell><Badge color="danger">{trip.ride_status}</Badge></TableCell>
											<TableCell>
