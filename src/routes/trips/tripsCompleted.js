...
const  TripsCompleted = ({match, trips, getTrips, isLoading, getTripCount, tripCount}) => {
	const [currentPage, setCurrentPage] = useState(1);
	const [postsPerPage] = useState(10);

useEffect(()=> {
	getTrips(1, 'completed');
	...
	return (
			<div className="table-wrapper">
				...
				{isLoading && <Spinner />}
				{!isLoading && <RctCollapsibleCard heading="Completed Trips" fullBlock>
								...
								<Fragment>
									{trips.length > 0 && trips.map((trip) => (
										<TableRow hover key={trip._id}>
											...
											<TableCell><Badge color="success">{trip.ride_status}</Badge></TableCell>
											<TableCell>
