 ...
 */
 ...
 const WalletsRefund = ({history, match,wallets, getWallets, getWalletsCount, getFundingBalance}) => {
   ...
   useEffect(() => {
	 if (pageFromQuery === undefined || wallets.length < 1) {
	   getWallets(currentPage, 4, "", true, "");
	   getWalletsCount(4, "", true, "");
	   getFundingBalance("", 4, "");
	 }
   }, []);
   ...
   return (
	 <div className="table-wrapper">
	   ...
	   <WalletTable status={4} heading={"Refunded Transactions"} />
	 </div>
   );
 };
 ...
 export default connect(mapStateToProps, mapDispatchToProps)(WalletsRefund);
