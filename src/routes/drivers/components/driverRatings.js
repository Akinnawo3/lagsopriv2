...
const DriverRatings = ({getUserRating, ratingsUser, loading, ratingCountUser, auth_id, ratingsUserAverage}) => {
  const [currentPage, setCurrentPage] = useState(1);

  const paginate = (pageNumber) => {
    setCurrentPage(pageNumber);
    getUserRating(pageNumber, "driver", auth_id);
    window.scrollTo(0, 0);
  };
      ...
      {!loading && ratingsUser?.length > 0 && (
        <RctCollapsibleCard>
                    ...
                    <TableRow hover key={key}>
                      ...
                      <TableCell>
                        <button type="button" className="rct-link-btn text-primary" title="view details">
                          <Link to={`/admin/ratings/${user.rating_id}`}>
                            <i className="ti-eye" />
                          </Link>
                        </button>
                      </TableCell>
          ...
          <div className="d-flex justify-content-end align-items-center mb-0 mt-3 mr-2">
            <Pagination activePage={currentPage} itemClass="page-item undo-folding" linkClass="page-link" itemsCountPerPage={20} totalItemsCount={ratingCountUser} onChange={paginate} />
          </div>
        </RctCollapsibleCard>
      )}
