...
const  ActiveDrivers = ({match,getDrivers, drivers, createDrivers, isLoading, getVehicles, vehicles}) => {
	...
	useEffect(()=> {
		if(searchData && drivers){
			setCurrentPage(1)
			const search = drivers.filter((user) => user.status === 1).filter(driver => {
	...
	return (
		<div className="table-wrapper">
			...
			{isLoading && <Spinner />}
			{!isLoading &&
			<RctCollapsibleCard heading="Active Drivers" fullBlock>
