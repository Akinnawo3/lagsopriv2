...
const  InactiveDrivers = ({match,getDrivers, drivers, createDrivers, isLoading, getVehicles, vehicles}) => {
	...
	useEffect(()=> {
		if(searchData && drivers){
			setCurrentPage(1)
			const search = drivers.filter((user) => user.status === 3).filter(driver => {
			...
			setPosts(drivers.filter((user) => user.status === 3).sort((a, b) => parseFloat(b.id) - parseFloat(a.id)))
		}
	},[searchData]);
	...
	return (
		<div className="table-wrapper">
			...
			{isLoading && <Spinner />}
			{!isLoading &&
			<RctCollapsibleCard heading="Inactive Drivers" fullBlock>
