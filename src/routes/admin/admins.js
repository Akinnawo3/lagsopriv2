...
const Admins = ({match, getAdmins, admins, updateAdmin, loading, deleteAdmin, adminCount, getAdminCount, createAdmin, loadingStatus, searchAdmins}) => {
  ...
  return (
    <div className="table-wrapper">
      ...
      {!loading && (
        <RctCollapsibleCard heading="Admins" fullBlock item={admins} currentPage={currentPage} totalCount={adminCount}>
          ...
          <div className="float-right">
            {!loading && admins.length > 0 && (
              <CSVLink data={excelExport} filename={"admins.csv"} className="btn-sm btn-outline-default mr-10 bg-primary text-white" target="_blank">
            ...
            > */}
            <button className="btn btn-success" onClick={(e) => opnAddNewUserModal(e)}>
              Add Personnel
            </button>
            {/* Create New Admin <i className="zmdi zmdi-plus"></i>
            </a> */}
          </div>
          {!loading && admins.length > 0 && (
                  ...
                  <Fragment>
                    {admins.map((admin, key) => (
                      <TableRow hover key={key}>
                        ...
                        <TableCell>
                          <button type="button" className="rct-link-btn" onClick={(e) => opnAddNewUserEditModal(admin.auth_id)}>
                            <i className="ti-pencil"></i>
                          </button>
                          <button type="button" className="rct-link-btn ml-lg-3 text-danger" onClick={() => onDelete(admin.auth_id)}>
          ...
          {!loading && admins.length > 0 && (
            <div className="d-flex justify-content-end align-items-center mb-0 mt-3 mr-2">
              <Pagination activePage={currentPage} itemClass="page-item" linkClass="page-link" itemsCountPerPage={20} totalItemsCount={adminCount} onChange={paginate} />
            </div>
          )}
        </RctCollapsibleCard>
      )}
      <Modal isOpen={addNewUserModal} toggle={() => onAddUpdateUserModalClose()}>
        <ModalHeader toggle={() => onAddUpdateUserModalClose()}>{editUser ? "Update Admin" : "Create New Admin"}</ModalHeader>
      ...
      <Modal isOpen={addNewUserModal1} toggle={() => onAddUpdateUserModalClose1()}>
        <ModalHeader toggle={() => onAddUpdateUserModalClose1()}>Upload Admin</ModalHeader>
        <ModalBody>
