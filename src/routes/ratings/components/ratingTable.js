      ...
      {!loading && ratings?.length > 0 && (
        <RctCollapsibleCard heading={header} fullBlock>
                ...
                <Fragment>
                  {ratings.map((user, key) => (
                    <TableRow hover key={key}>
                      <TableCell>
                        {user?.receiver[0]?.first_name} {user?.receiver[0]?.last_name}
                      </TableCell>
                      <TableCell>
                        <StarRatings
                          rating={user.rating}
          ...
          <div className="d-flex justify-content-end align-items-center mb-0 mt-3 mr-2">
            <Pagination activePage={currentPage} itemClass="page-item" linkClass="page-link" itemsCountPerPage={20} totalItemsCount={ratingCount} onChange={paginate} />
          </div>
        </RctCollapsibleCard>
      )}
