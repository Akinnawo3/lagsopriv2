...
const ContactUs = ({contactUs, getContactUs, contactUsCount, getContactUsCount, loading, match}) => {
  ...
  console.log(contactUsCount);

  
  return (
    <div className="table-wrapper">
      <PageTitleBar title={"Contact Us"} match={match} />
      {!loading && contactUs?.length > 0 && (
        <RctCollapsibleCard heading="Contact us" fullBlock>
                ...
                <Fragment>
                  {contactUs.map((sup, key) => (
                    <TableRow hover key={key}>
                      ...
                      <TableCell>{calculatePostDate(sup.createdAt)}</TableCell>
                      <TableCell>
          ...
          <div className="d-flex justify-content-end align-items-center mb-0 mt-3 mr-2">
            <Pagination activePage={currentPage} itemClass="page-item" linkClass="page-link" itemsCountPerPage={20} totalItemsCount={contactUsCount} onChange={paginate} />
          </div>
        </RctCollapsibleCard>
      )}
