...
const  UnresolvedSupport = ({match, support, getSupport, getTicketTypes, ticketTypes, loading}) => {
    ...
    useEffect(()=> {
        if(support.length > 0) {
            setPosts(support.sort((a, b) => parseFloat(b.id) - parseFloat(a.id)).filter(data => data.status == 5))
        }
    },[support])
    ...
    return (
        <div className="table-wrapper">
            ...
            {loading && <Spinner />}
            {!loading &&  <RctCollapsibleCard heading="Unresolved Tickets" fullBlock>
