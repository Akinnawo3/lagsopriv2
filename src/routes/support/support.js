...
const Support = ({history, match, getSupportTickets, getSupportTicketsCount, support}) => {
  ...
  useEffect(() => {
    if (pageFromQuery === undefined || support.length < 1) {
      getSupportTickets(currentPage, "", true);
  ...
  return (
    <div className="table-wrapper">
      ...
      <SupportTable support_type={""} header={"All Tickets"} />
    </div>
  );
};
