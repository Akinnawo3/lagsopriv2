...
const NewSupport = ({history, match, getSupportTickets, getSupportTicketsCount, support}) => {
  ...
  useEffect(() => {
    if (pageFromQuery === undefined || support.length < 1) {
      getSupportTickets(currentPage, 0, true);
      getSupportTicketsCount(1);
    }
  }, []);
  ...
  return (
    <div className="table-wrapper">
      ...
      <SupportTable support_type={1} header={"New Tickets"} />
    </div>
  );
};
