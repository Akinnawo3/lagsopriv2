...
const  ActivePassengers = ({match, getPassengers, passengers, loading, changePassengerStatus, loadingStatus}) => {
	...
	useEffect(()=> {
		if(searchData && passengers){
			setCurrentPage(1)
			const search = passengers.filter((user) => user.phoneNumberStatus === 1).filter(passenger => {
			...
			setPosts(passengers.filter((user) => user.phoneNumberStatus === 1).sort((a, b) => parseFloat(b.id) - parseFloat(a.id)))
		}
	},[searchData]);
	...
	useEffect(()=> {
		if(passengers) {
			setPosts(passengers.filter((user) => user.phoneNumberStatus === 1).sort((a, b) => parseFloat(b.id) - parseFloat(a.id)))
			let result = passengers.filter((user) => user.phoneNumberStatus === 1).map(passenger=> {
				return {
	...
	return (
			<div className="table-wrapper">
				...
				{loading && <Spinner />}
				{!loading &&
				<RctCollapsibleCard heading="Active Passengers" fullBlock>
								...
								<Fragment>
									{posts && currentPosts.map((passenger, key) => (
										<TableRow hover key={key}>
											...
											<TableCell>
												<button type="button" className="rct-link-btn text-primary" title="view details"><Link to={`/admin/passengers/${passenger.authId}`}><i className="ti-eye"/></Link></button>
												{/*{passenger.phoneNumberStatus === 0 &&*/}
...
export default connect( mapStateToProps, mapDispatchToProps) (ActivePassengers);
