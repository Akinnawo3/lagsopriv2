...
const RefundPaymentService = ({history, match, getPayments, getPaymentsCount, payments, paymentsCount, getPaymentsServiceBalance}) => {
  ...
  useEffect(() => {
    if (pageFromQuery === undefined || payments.length < 1) {
      getPayments(currentPage, 4, "", true);
      getPaymentsCount(4);
      getPaymentsServiceBalance(3);
    }
  }, []);
  ...
  return (
    <div className="table-wrapper">
      ...
      <PaymentServiceTable status={4} header={"Refund Payments"} />
    </div>
  );
};
...
export default connect(mapStateToProps, mapDispatchToProps)(RefundPaymentService);
