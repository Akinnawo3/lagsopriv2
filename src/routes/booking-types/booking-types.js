...
const  BookingTypes = (props) => {
    ...
    return (
        <div className="table-wrapper">
            ...
            {loading && <Spinner />}
            {!loading &&
            <RctCollapsibleCard heading="BookingTypes" fullBlock>
                ...
                <div className="float-right">
                    ...
                    <a href="#" onClick={(e) => opnAddNewUserModal(e)} color="primary" className="caret btn-sm mr-10">Create New BookingType <i className="zmdi zmdi-plus"></i></a>
                </div>
                <div className="table-responsive" style={{minHeight: "50vh"}}>
                    <Table>
                        ...
                        <TableBody>
                            <Fragment>
                                {posts && currentPosts.map((bookingType, key) => (
                                    <TableRow hover key={key}>
                                        <TableCell>{bookingType.trip_name}</TableCell>
                                        <TableCell>{bookingType.trip_description}</TableCell>
                                        <TableCell>
                                            <button type="button" className="rct-link-btn" onClick={(e) => opnAddNewUserEditModal(bookingType.id)}><i className="ti-pencil"></i></button>
                                            <button type="button" className="rct-link-btn ml-lg-3 text-danger" onClick={() => onDelete(bookingType.id)}><i className="ti-close"></i></button>
                                        </TableCell>
                                    </TableRow>
                                ))}
                            </Fragment>
            ...
            <Modal isOpen={addNewUserModal} toggle={() => onAddUpdateUserModalClose()}>
                <ModalHeader toggle={() => onAddUpdateUserModalClose()}>
                    {editUser ? 'Update Booking Type': 'Create New Booking Type'}
                </ModalHeader>
