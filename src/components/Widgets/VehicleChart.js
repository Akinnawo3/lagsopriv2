...
const VehicleChart = ({data}) => {
  ...
  const [vehicleCountPartnerAssigned, setVehicleCountPartnerAssigned] = useState(0);
  const [vehicleCountUnassigned, setVehicleCountUnassigned] = useState(0);

  useEffect(() => {
    getVehicleCount();
    getVehicleAssigned();
    getVehiclePartnerAssigned();
  ...
  const getVehiclePartnerAssigned = async () => {
    try {
      const res = await axios.get(`${api.vehicles}/v1.1/admin/vehicles?component=count&partner_assign=1`);
      if (res.data.status === "error") {
        NotificationManager.error(res.data.msg);
      } else {
        setVehicleCountPartnerAssigned(res.data.data.total ? res.data.data.total : 0);
      }
    } catch (err) {}
  };
          ...
          <div className="w-100">
            <div className="d-flex justify-content-between w-100">
              <h3 className="mb-15 fw-semi-bold">
                <IntlMessages id="widgets.vehicles" />
              </h3>
            ...
            <div className="d-flex">
              ...
              <div className="mr-50">
                ...
                <CountUp separator="," className="counter-point" start={0} end={vehicleCountUnassigned} duration={5} useEasing={true} />
              </div>
              <div className="">
                <span className="fs-14 d-block">
                  <IntlMessages id="widgets.total" />
                </span>
                <CountUp separator="," className="counter-point" start={0} end={vehicleCount} duration={5} useEasing={true} />
              </div>
            </div>
          </div>
        </div>
