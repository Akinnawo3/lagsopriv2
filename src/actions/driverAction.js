...
export const getDriversCount =
  ...
  async (dispatch) => {
    try {
      const res = await axios.get(
        `${api.user}/v1.1/admin/users?user_type=driver&driver_account_status=${status}&component=count`
      );
      if (res.data.status === "error") {
        NotificationManager.error(res.data.msg);
      } else {
    ...
    if (res.data.status === "error") {
      ...
      if (res.data.data.driver_data?.vehicle_id) {
        res2 = await axios.get(
          `${api.vehicles}/v1.1/vehicles/${res.data.data.driver_data?.vehicle_id}`
        );
        await dispatch({
          type: VEHICLE,
...
export const changeDriverStatus =
  (auth_id, driver_status, driverData, message_type, subject) =>
  async (dispatch) => {
...
export const changeDriverCategory =
  (auth_id, category, driverData, message_type, subject) =>
  async (dispatch) => {
...
export const verifyID =
  (auth_id, verification_status, verification_name) => async (dispatch) => {
    const body = {
...
export const searchDrivers = (searchData, status) => async (dispatch) => {
  try {
    dispatch(startStatusLoading());
    const res = await axios.get(
      `${api.user}/v1.1/admin/users?user_type=driver&q=${searchData}&driver_account_status=${status}`
    );
    if (res.data.status === "error") {
      NotificationManager.error(res.data.msg);
    } else {
      const res2 = await axios.get(
        `${api.user}/v1.1/admin/users?user_type=driver&driver_account_status=${status}&component=count&q=${searchData}`
      );
      dispatch({
        type: DRIVERS_COUNT,
...
export const sendDriverMessage =
  (driverData, message_type, subject) => async (dispatch) => {
    const body = {
      type: "generic",
...
export const getDriversLocation = (longitude, latitude) => async (dispatch) => {
  try {
    const res = await axios.get(
      `${api.user}/v1.1/admin/user-location/?latitude=${latitude}&longitude=${longitude}&user_type=driver`
    );
    if (res.data.status === "error") {
      NotificationManager.error(res.data.msg);
    } else {
...
export const getDriverLocation =
  (driverData, vehicleData) => async (dispatch) => {
    try {
      dispatch(startStatusLoading());
      let res = vehicleData ? vehicleData : {};
      if (!vehicleData && driverData.driver_data.vehicle_id) {
        res = await axios.get(
          `${api.vehicles}/v1.1/vehicles/${driverData.driver_data.vehicle_id}`
        );
      }
      dispatch({
        ...
        payload: [
          {
            ...
            plate_no: res.data.data.car_number_plate
              ? res.data.data.car_number_plate
              : "NA",
          },
        ],
      });
