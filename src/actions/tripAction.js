...
export const getCancelledTrips = (pageNo, spinner) => async (dispatch) => {
  try {
    spinner && dispatch(startLoading());
    !spinner && dispatch(startStatusLoading());
    const res = await axios.get(`${api.trip}/v1.1/trips/cancelled-trips?item_per_page=20&page=${pageNo}`);
    if (res.data.status === "error") {
      NotificationManager.error(res.data.msg);
    } else {
      dispatch({
...
export const getCancelledTripCount = () => async (dispatch) => {
  try {
    const res = await axios.get(`${api.trip}/v1.1/trips/cancelled-trips?component=count`);
    if (res.data.status === "error") {
      NotificationManager.error(res.data.msg);
    } else {
      dispatch({
...
export const getTrip = (trip_id, spinner, trip_status) => async (dispatch) => {
  try {
    ...
    !spinner && dispatch(startStatusLoading());
    const res =
      trip_status == "on_ride" || trip_status == "waiting" ? await axios.get(`${api.trip}/v1.1/trips/${trip_id}/?trip_status=${trip_status}`) : await axios.get(`${api.trip}/v1.1/trips/${trip_id}`);
    if (res.data.status === "error") {
      NotificationManager.error(res.data.msg);
    } else {
      dispatch({
...
export const searchTrip =
  ...
  async (dispatch) => {
    try {
      ...
      const res = await axios.get(`${api.trip}/v1.1/trips/?trip_ref=${trip_ref}&trip_status=${status}`);
      if (res.data.status === "error") {
        NotificationManager.error(res.data.msg);
      } else {
...
export const getActualAddress = (lat, lng, spinner) => async (dispatch) => {
  try {
    const res = await fetch("https://maps.googleapis.com/maps/api/geocode/json?address=" + "6.601838,3.3514863" + "&key=" + "AIzaSyCw_5YoOp78lvq1Dgfri-TnDjRSf1cguf0")
...
export const getDriversPerformance =
  ...
  async (dispatch) => {
    try {
      spinner && dispatch(startLoading());
      !spinner && dispatch(startStatusLoading());
      // const res = await axios.get(`https://staging-server.lagosride.com/tripreporting-service/v1.1/analytics/compliance`);
